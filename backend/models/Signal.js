const mongoose = require('mongoose');

const SignalSchema = new mongoose.Schema({
    pair: {
        type: String,
        required: [true, 'Please select a currency pair'],
        enum: [
            // ===== MAJOR PAIRS =====
            'EUR/USD',
            'GBP/USD',
            'USD/JPY',
            'AUD/USD',
            'USD/CAD',
            'USD/CHF',
            'NZD/USD',
            
            // ===== MINOR PAIRS =====
            'EUR/GBP',
            'EUR/JPY',
            'EUR/CHF',
            'GBP/JPY',
            'GBP/CHF',
            'GBP/AUD',
            'AUD/JPY',
            'AUD/CHF',
            'AUD/NZD',
            'CAD/JPY',
            'CHF/JPY',
            'NZD/JPY',
            'NZD/CHF',
            'NZD/CAD',
            
            // ===== EXOTIC PAIRS =====
            'EUR/SEK',
            'EUR/NOK',
            'EUR/DKK',
            'GBP/SEK',
            'GBP/NOK',
            'USD/SEK',
            'USD/NOK',
            'USD/DKK',
            'USD/TRY',
            'USD/ZAR',
            'USD/SGD',
            'USD/HKD',
            'USD/MXN',
            'USD/PLN',
            'USD/CZK',
            'USD/HUF',
            'USD/ILS',
            'USD/KRW',
            'USD/TWD',
            'USD/THB',
            'USD/CNY',
            'USD/INR',
            'USD/BRL',
            'USD/RUB',
            
            // ===== SOUTH AFRICAN ZAR PAIRS =====
            'EUR/ZAR',
            'GBP/ZAR',
            'AUD/ZAR',
            'NZD/ZAR',
            'JPY/ZAR',
            'CHF/ZAR',
            'CAD/ZAR',
            'SEK/ZAR',
            'NOK/ZAR',
            'DKK/ZAR',
            'CNY/ZAR',
            'INR/ZAR',
            'TRY/ZAR',
            'PLN/ZAR',
            'CZK/ZAR',
            'HUF/ZAR',
            'SGD/ZAR',
            'HKD/ZAR',
            'MXN/ZAR',
            'BRL/ZAR',
            'RUB/ZAR',
            
            // ===== GOLD & COMMODITIES =====
            'XAU/USD', // Gold
            'XAG/USD', // Silver
            'XAU/ZAR', // Gold in ZAR
            'XAG/ZAR', // Silver in ZAR
            
            // ===== CRYPTO PAIRS (if you want to include) =====
            'BTC/USD',
            'ETH/USD',
            'BTC/ZAR',
            'ETH/ZAR'
        ]
    },
    action: {
        type: String,
        required: [true, 'Please select BUY or SELL'],
        enum: ['BUY', 'SELL']
    },
    entry: {
        type: Number,
        required: [true, 'Please provide entry price'],
        min: 0
    },
    takeProfit: {
        type: Number,
        required: [true, 'Please provide take profit level'],
        min: 0
    },
    stopLoss: {
        type: Number,
        required: [true, 'Please provide stop loss level'],
        min: 0
    },
    confidence: {
        type: String,
        enum: ['HIGH', 'MEDIUM', 'LOW'],
        default: 'MEDIUM'
    },
    analysis: {
        type: String,
        maxlength: 500,
        default: ''
    },
    status: {
        type: String,
        enum: ['active', 'closed', 'cancelled'],
        default: 'active'
    },
    createdAt: {
        type: Date,
        default: Date.now
    },
    expiresAt: {
        type: Date,
        default: () => new Date(+new Date() + 24*60*60*1000) // 24 hours
    },
    sentTo: [{
        type: mongoose.Schema.Types.ObjectId,
        ref: 'User'
    }]
});

// Index for faster queries
SignalSchema.index({ pair: 1, createdAt: -1 });
SignalSchema.index({ status: 1 });
SignalSchema.index({ createdAt: -1 });
SignalSchema.index({ 'sentTo': 1 });

module.exports = mongoose.model('Signal', SignalSchema);